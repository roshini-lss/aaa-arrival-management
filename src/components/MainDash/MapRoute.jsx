...
function App() {
    ...
    async function calculateRoute(coord) {
        ...
        const destination = {
            lat: parseFloat(coord?.destination?.lat || destinationLatRef.current.value),
            lng: parseFloat(coord?.destination?.lng || destinationLngRef.current.value),
        }
        ...
        const predictedDurationInSeconds = googleDurationInSeconds + 300
   ...
   useEffect(() => {
    ...
    if(isLoaded&&(state.dest_latitude!=null && state.dest_longitude!=null)) calculateRoute({
        destination:{lat:state.dest_latitude, lng:state.dest_longitude}
    });
