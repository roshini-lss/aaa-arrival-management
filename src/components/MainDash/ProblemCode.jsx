...
const ProblemCode = () => {
  const listData = ["Battery_Jump", "Battery_Service", "Extrication_Recovery", "Flat_Tire_No_Spare", "Tow_Accident", "Tow_Motorcycle", "Tow_Mechanical", "Lockout"]
  const region = useParams();
  const regions = region?.val?.toUpperCase();
  const  batteryJump = (service) => {
   const details = { totalNumberOfOrders: allData.filter(
      (item) =>
        item.State === regions &&
        item.description_of_the_problem_code === service
    ).length,
    assingedPTA: allData.filter(item => item.State === regions &&  item.description_of_the_problem_code === service && item.pta_truck_predicted > 0).length,
    delays: allData.filter(item => item.State === regions &&  item.description_of_the_problem_code === service && item.pta_truck > item.pta_truck_predicted).length,
}
return details
  };
  ...
  const cardsData = [
    {
      ...
      unassigned: serviceData.Battery_Jump.totalNumberOfOrders - serviceData.Battery_Jump.assingedPTA || 0,
    },
    {
      ...
      unassigned: serviceData.Battery_Service.totalNumberOfOrders - serviceData.Battery_Service.assingedPTA
    },
    {
      ...
      unassigned: serviceData.Extrication_Recovery.totalNumberOfOrders - serviceData.Extrication_Recovery.assingedPTA
    },
    {
      ...
      unassigned: serviceData.Flat_Tire_No_Spare.totalNumberOfOrders - serviceData.Flat_Tire_No_Spare.assingedPTA
    },
    {
      title: "Tow Accident",
      img: towAccident,
      ...
      unassigned: serviceData.Tow_Accident.totalNumberOfOrders - serviceData.Tow_Accident.assingedPTA
    },
    {
      ...
      unassigned: serviceData.Tow_Motorcycle.totalNumberOfOrders - serviceData.Tow_Motorcycle.assingedPTA
    },
    {
      ...
      unassigned: serviceData.Tow_Mechanical.totalNumberOfOrders - serviceData.Tow_Mechanical.assingedPTA
    },
    {
      title: "Lockout",
      img: lockout,
      ...
      unassigned: serviceData.Lockout.totalNumberOfOrders - serviceData.Lockout.assingedPTA
    },
  ];
  return (
